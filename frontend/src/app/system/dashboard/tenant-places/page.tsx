...
export default function TenantPlacesPage() {
  ...
  const list = useResourceList<TenantPlaceRow>("/system/tenant-places", {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await api(`/system/tenant-places/${editing.id}`, { method: "PATCH", body });
        toast.success("Place updated");
      } else {
        await api("/system/tenant-places", { method: "POST", body });
  ...
  async function onDelete() {
    ...
    setDeleting(true);
    try {
      await api(`/system/tenant-places/${deleteTarget.id}`, { method: "DELETE" });
  ...
  async function onRestore(row: TenantPlaceRow) {
    setRestoringId(row.id);
    try {
      await api(`/system/tenant-places/${row.id}/restore`, { method: "PATCH" });
  ...
  return (
    <>
      <ResourceTable<TenantPlaceRow>
        title="Places"
        description="Manage places for tenant businesses."
        columns={columns}
        rows={list.rows}
        error={list.error}
        page={list.page}
          ...
          <>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="placeName">Place Name</SelectItem>
          ...
          {mapPickerOpen ? (
            ...
            <form onSubmit={onSubmit}>
              <DialogHeader>
                ...
                <DialogDescription>A place associated with a tenant business.</DialogDescription>
              </DialogHeader>
      ...
      <ConfirmDialog
        ...
        title="Delete place"
        description={`This will soft-delete "${deleteTarget?.placeName}".`}
        loading={deleting}
        onConfirm={onDelete}
      />
    </>
