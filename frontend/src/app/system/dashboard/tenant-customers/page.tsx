...
export default function TenantCustomersPage() {
  ...
  const list = useResourceList<TenantCustomerRow>(
    viewingDeleted ? "/system/tenant-customers/deleted" : "/system/tenant-customers",
    {
      status: viewingDeleted ? undefined : statusFilter || undefined,
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await api(`/system/tenant-customers/${editing.id}`, { method: "PATCH", body });
        toast.success("Customer updated");
      } else {
        await api("/system/tenant-customers", { method: "POST", body });
  ...
  async function onDelete() {
    ...
    setDeleting(true);
    try {
      await api(`/system/tenant-customers/${deleteTarget.id}`, { method: "DELETE" });
  ...
  async function onRestore(row: TenantCustomerRow) {
    ...
    try {
      await api(`/system/tenant-customers/${row.id}/restore`, { method: "PATCH" });
  ...
  async function onChangeStatus() {
    ...
    try {
      await api(`/system/tenant-customers/${statusTarget.id}/status`, {
        method: "PATCH",
      ...
      toast.error(error instanceof ApiError ? error.message : "Status update failed");
    } finally {
  ...
  async function onExport() {
    ...
    try {
      ...
      if (statusFilter && !viewingDeleted) params.set("status", statusFilter);
      if (businessFilter) params.set("tenantBusinessId", businessFilter);
      const headers: Record<string, string> = { "x-device-type": "website" };
      const token = getAccessToken();
      if (token) headers.Authorization = `Bearer ${token}`;
      const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000/api/v1";
      const res = await fetch(`${apiUrl}/system/tenant-customers/export?${params.toString()}`, { headers });
  ...
  async function onImportSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (token) headers.Authorization = `Bearer ${token}`;
      const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000/api/v1";
      const res = await fetch(`${apiUrl}/system/tenant-customers/import`, {
        method: "POST",
        headers,
        body: formData,
      });
  ...
  return (
    <>
      <ResourceTable<TenantCustomerRow>
        title="Customers"
        description="Manage tenant business customers across all businesses."
        columns={columns}
        rows={list.rows}
        error={list.error}
        page={list.page}
          ...
          <>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="fName">Name</SelectItem>
                <SelectItem value="customerCode">Code</SelectItem>
          ...
          {viewTarget && (
            <div className="grid gap-4 py-2">
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-3 rounded-md border p-4 sm:grid-cols-2">
                ...
                <DetailField label="Business" value={viewTarget.tenantBusiness?.name} />
