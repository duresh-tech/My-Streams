...
export default function TenantMailConfigPage() {
  ...
  const list = useResourceList<TenantMailConfigRow>("/system/tenant-mail-config", {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await api(`/system/tenant-mail-config/${editing.id}`, { method: "PATCH", body });
        toast.success("Mail config updated");
      } else {
        await api("/system/tenant-mail-config", { method: "POST", body });
  ...
  async function onDelete() {
    ...
    setDeleting(true);
    try {
      await api(`/system/tenant-mail-config/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <>
      <ResourceTable<TenantMailConfigRow>
        title="Mail Config"
        description="Manage SMTP mail configuration for tenant businesses."
        columns={columns}
        rows={list.rows}
        error={list.error}
        page={list.page}
          ...
          <>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="mailHost">Host</SelectItem>
              </SelectContent>
            </Select>
            <Button
              type="button"
        ...
        <DialogContent>
          <form onSubmit={onSubmit}>
            <DialogHeader>
              ...
              <DialogDescription>SMTP mail configuration for a tenant business.</DialogDescription>
            </DialogHeader>
